...
const CreateTest = () => {
    ...
    const handleAIQuestions = async () => {
        if (!examData.subject || !examData.title) {
            toast.error("Please set subject and title first");
            return;
        }
        ...
        try {
            ...
            const formattedQuestions = res.data.questions.map(q => ({
                text: q.text || '',
                type: q.type || 'mcq',
                options: q.options && q.options.length > 0 ? q.options : (q.type === 'mcq' ? ['', '', '', ''] : []),
                correctOptions: q.correctOptions || [0],
            ...
            toast.success(`${formattedQuestions.length} questions generated by AI`, { id: loadingToast });
        } catch (error) {
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-2 border-b border-white/5">
                        ...
                        <div className="flex items-center gap-2">
                            <button
                                ...
                                className="px-4 py-2 bg-linear-to-r from-[#D4AF37] to-[#996515] text-white text-[10px] font-black uppercase tracking-widest rounded-xl shadow-lg shadow-gold-600/20 hover:scale-105 transition-all"
                            >
                                AI Generate
                            </button>
                            <button
